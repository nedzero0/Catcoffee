...
<head>
	...
	<title>ad-dinnerTaShow.jsp</title>
<script>
	/*图片-删除*/
	/*$(function () {
		$(".delId").click(function () {
		return 	confirm('确认要删除吗？',function(index){
		...
		<article class="cl pd-20">
			...
			<div class="mt-20">
				<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer"><div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer"><div id="DataTables_Table_0_filter" class="dataTables_filter"></div><table class="table table-border table-bordered table-bg table-hover table-sort dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
					<thead>
					<tr class="text-c" role="row"><th width="40" class="sorting_disabled" rowspan="1" colspan="1" aria-label="" style="width: 40px;"><input name="" type="checkbox" value=""></th><th width="80" class="sorting_desc" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="ID: 升序排列" style="width: 80px;" aria-sort="descending">ID</th><th width="100" class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="分类: 升序排列" style="width: 100px;">餐桌名称</th><th width="150" class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="更新时间: 升序排列" style="width: 149px;">预定时间</th><th width="60" class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="发布状态: 升序排列" style="width: 60px;">餐桌状态</th><th width="100" class="sorting_disabled" rowspan="1" colspan="1" aria-label="操作" style="width: 101px;">操作</th></tr>
					</thead>

					<tbody>

					<c:forEach items="${requestScope.dinnerTaList}" var="dinner">
					  <tr class="text-c" role="row">
						<td><input name="" type="checkbox" value=""></td>
						<td class="sorting_1">${dinner.id}</td>
						<td>${dinner.tableName}</td>

						<td class="td-date">${dinner.orderDate}</td>

						  <c:if test="${dinner.tableStatus>0}">
						<td class="td-status"><span class="label label-success radius">预定状态</span></td>
						  </c:if>
						  <c:if test="${dinner.tableStatus==0}">
							  <td class="td-status"><span class="label label-success radius" style="background-color: #999999">空闲状态</span></td>
						  </c:if>

						  <td class="td-manage">
							  <a style="text-decoration:none" class="updateStus" onclick="picture_stop(this,${dinner.id})" href="javascript:;" title="${dinner.id}" name="${dinner.id}">
								  <i class="Hui-iconfont"></i>
							  </a>
							  <a style="text-decoration:none" class="ml-5" onclick="picture_edit('图库编辑','picture-add.html','10001')" href="javascript:;" title="编辑">
								  <i class="Hui-iconfont"></i>
							  </a>
							  <a style="text-decoration:none" class="ml-5 delId" onclick="dinner_del(this,${dinner.id})" href="javascript:;">
								  <i class="Hui-iconfont"></i>
...
<script type="text/javascript">
	...
	function dinner_del(obj,id){
		layer.confirm('确认要删除吗？',function(index){
			...
			*/
			//alert(id);
			$.getJSON("http://localhost:8770/Catcoffee/dinTaServlet","action=delDinnerTa&dinnerId="+id,
					function (data) {
